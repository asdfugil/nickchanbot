...
function statsCommand(receivedMessage) {
  ...
  client.shard.broadcastEval("process.memoryUsage().heapUsed").then(mem => {
    const memUsage = mem.reduce((prev, mems) => prev + mems, 0);
    client.shard.broadcastEval("this.users.size").then(discorduser => {
      const users = discorduser.reduce((prev, userCount) => prev + userCount, 0);
      client.shard.broadcastEval("this.guilds.size").then(serv => {
        const servers = serv.reduce((prev, guildCount) => prev + guildCount, 0);
        client.shard.broadcastEval("this.channels.size").then(chann => {
          const channels = chann.reduce(
            (prev, channelCount) => prev + channelCount,
            0
          );
          receivedMessage.channel.send(
            `= STATISTICS =
            • Mem Usage  :: ${(memUsage / 1024 / 1024).toFixed(2)} MB
            • Uptime     :: ${duration}
            • Users      :: ${users}
            • Servers    :: ${servers}
            • Channels   :: ${channels}
