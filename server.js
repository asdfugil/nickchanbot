...
child.stdout.on("data",chunk => hook.send(chunk.toString(),{split:true}).catch(console.error))
child.stderr.on("data",chunk => hook.send(chunk.toString(),{split:true}).catch(console.error))
child.stdout.on("data",chunk => {
console.log(chunk.substring(0,chunk.length-1))
})
child.stderr.on("data",chunk => console.error(chunk.substring(0,chunk.length-1)))